...
function update_gateway_fee2($vars)
{
    ...
    $taxGatewayFees = Capsule::table('tbladdonmodules')->where('module', 'sm_gateway_fees')->where('setting', 'tax_sm_gateway_fees')->value('value');
    ...
    Capsule::table('tblinvoiceitems')
        ...
        ->where('notes', 'sm_gateway_fees')
        ->delete();
    log_to_file("DB Delete: tblinvoiceitems where invoiceid=" . $invoiceId . " and notes='sm_gateway_fees'");
    ...
    if($fee1 > 0.00 || $fee2 > 0.00) {
        Capsule::table('tblinvoiceitems')->insert([
            'invoiceid' => $invoiceId,
            'type' => 'Item',
            'description' => "ScoobyMedia Customer Gateway Fee ({$fee2}% / {$fee1}{$currencysuffix})",
            'amount' => $totalFee,
            'taxed' => $taxGatewayFees?1:0, // Edit to 0 if you don't want to make the amount excluding tax
            'notes' => 'sm_gateway_fees'
        ]);
